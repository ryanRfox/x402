...
/**
 * Formats a Mezo USD (18-decimal) base-unit amount as a human readable mUSD string.
 *
 * @param amount - Token amount in mUSD base units (18 decimals).
 * @returns Formatted string like `12.345000 mUSD (12345000000000000000 base units)`.
 */
export function fmtMUsd(amount: bigint | string | undefined): string {
  if (amount === undefined) return "—";
  const n = typeof amount === "bigint" ? amount : BigInt(amount);
  return `${formatUnits(n, 18)} mUSD (${n} base units)`;
}
