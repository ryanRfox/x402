...
import { mezoTestnet } from "./src/chain";
import {
...
const NETWORK = "eip155:31611" as const;
...
async function main(): Promise<PhaseResult[]> {
  ...
  console.log(`  network              ${NETWORK} (Mezo Testnet)`);
  ...
  const publicClient = createPublicClient({ chain: mezoTestnet, transport: http(env.rpcUrl) });
  const mUsdBefore = await readErc20Balance(publicClient, MEZO_MUSD, env.payerAccount.address);
  console.log(`Payer mUSD balance: ${fmtMUsd(mUsdBefore)}\n`);
  if (mUsdBefore < parseUnits("0.10", 18)) {
    console.warn(
      "WARNING: payer mUSD balance is below 0.10 — Mezo Testnet faucet: https://faucet.test.mezo.org/\n",
    );
  }
  ...
  try {
    ...
    const requirementsForChannel1 = {
      ...
      extra: {
        ...
        withdrawDelay: WITHDRAW_DELAY_SECS,
        // Mezo's mUSD does NOT support EIP-3009; force the Permit2 deposit path.
    ...
    let channel1Id: Hex;
    {
      ...
      printDiff("onchain.balance", fmtMUsd(beforeOnchain.balance), "(pending)");
      printDiff("onchain.totalClaimed", fmtMUsd(beforeOnchain.totalClaimed), "(pending)");
      ...
      console.log("\n  After first request:");
      printDiff("onchain.balance", fmtMUsd(beforeOnchain.balance), fmtMUsd(afterOnchain.balance));
      printDiff(
        "onchain.totalClaimed",
        fmtMUsd(beforeOnchain.totalClaimed),
        fmtMUsd(afterOnchain.totalClaimed),
      );
      ...
      assertInvariant(
        ...
        `balance=${fmtMUsd(afterOnchain.balance)}`,
      );
      assertInvariant(
      ...
      results.push({
        ...
        actual: `balance=${fmtMUsd(afterOnchain.balance)}, chargedCumulative=${fmtMUsd(serverState?.chargedCumulativeAmount)}`,
        passed: true,
      });
    }
    ...
    // Phase 2: steady-state.
    {
      ...
      for (let i = 1; i <= STEADY_REQUESTS; i++) {
        ...
        console.log(
          `  request ${i}/${STEADY_REQUESTS}: voucher=${fmtMUsd(localState?.signedMaxClaimable ?? localCharged)} ` +
            `server.charged=${fmtMUsd(serverCharged)} onchain.totalClaimed=${fmtMUsd(onchainState.totalClaimed)}`,
        );
      ...
      results.push({
        ...
        actual: `final cumulative=${fmtMUsd(lastServerCharged)}; onchain.totalClaimed=0`,
        passed: true,
      });
    }
    ...
    // Phase 3: state-loss recovery via corrective 402.
    {
      ...
      console.log(
        `  server.chargedCumulative still: ${fmtMUsd(serverBefore?.chargedCumulativeAmount)}`,
      );
      ...
      results.push({
        ...
        actual: `cumulative ${fmtMUsd(serverBefore?.chargedCumulativeAmount)} -> ${fmtMUsd(serverAfter?.chargedCumulativeAmount)}`,
        passed: true,
      });
    }
    ...
    // Phase 4: cooperative refund.
    {
      ...
      console.log(
        `  onchain.totalClaimed: ${fmtMUsd(onchainAfterClaim.totalClaimed)}; ` +
          `receivers.totalSettled: ${fmtMUsd(receiverTotalsAfterClaim.totalSettled)}`,
      );

      console.log("\n  Calling scheme.refund(url) — full refund of remaining channel balance…");
      const mUsdBeforeRefund = await readErc20Balance(
        publicClient,
        MEZO_MUSD,
      ...
      const mUsdAfterRefund = await readErc20Balance(
        publicClient,
        MEZO_MUSD,
        env.payerAccount.address,
      );
      ...
      printDiff(
        "onchain.balance",
        fmtMUsd(onchainAfterClaim.balance),
        fmtMUsd(onchainAfterRefund.balance),
      );
      printDiff(
        "onchain.totalClaimed",
        fmtMUsd(onchainAfterClaim.totalClaimed),
        fmtMUsd(onchainAfterRefund.totalClaimed),
      );
      printDiff("payer mUSD balance", fmtMUsd(mUsdBeforeRefund), fmtMUsd(mUsdAfterRefund));
      ...
      assertInvariant(
        "payer mUSD increased after refund",
        mUsdAfterRefund > mUsdBeforeRefund,
        `${mUsdBeforeRefund} -> ${mUsdAfterRefund}`,
      );
      results.push({
        phase: 4,
        name: "Cooperative refund",
        expected: "server claims, signs refund, available escrow returns to payer",
        actual: `available ${fmtMUsd(onchainAfterClaim.balance - onchainAfterClaim.totalClaimed)} -> 0; payer +${fmtMUsd(mUsdAfterRefund - mUsdBeforeRefund)}`,
        passed: true,
      });
    }
    ...
    // Phase 5: unilateral withdraw after server goes dark.
    {
      ...
      console.log(
        `  channel funded: balance=${fmtMUsd(stateBeforeKill.balance)} totalClaimed=${fmtMUsd(stateBeforeKill.totalClaimed)}`,
      );
      ...
      const wallet = createMezoWallet(env.payerAccount, env.rpcUrl);
      ...
      console.log(
        `  pendingWithdrawals: amount=${fmtMUsd(stateAfterInit.pendingWithdrawAmount)} initiatedAt=${stateAfterInit.pendingWithdrawInitiatedAt} (epoch s)`,
      );
      assertInvariant(
      ...
      if (env.waitFullWithdraw) {
        ...
        const mUsdBeforeFinalize = await readErc20Balance(
          publicClient,
          MEZO_MUSD,
          env.payerAccount.address,
        );
        ...
        const mUsdAfterFinalize = await readErc20Balance(
          publicClient,
          MEZO_MUSD,
          env.payerAccount.address,
        );
        printDiff(
          "onchain.balance",
          fmtMUsd(stateAfterInit.balance),
          fmtMUsd(stateAfterFinalize.balance),
        );
        printDiff(
          "onchain.pendingWithdraw",
          fmtMUsd(stateAfterInit.pendingWithdrawAmount),
          fmtMUsd(stateAfterFinalize.pendingWithdrawAmount),
        );
        printDiff("payer mUSD", fmtMUsd(mUsdBeforeFinalize), fmtMUsd(mUsdAfterFinalize));
        ...
        assertInvariant(
          ...
          mUsdAfterFinalize > mUsdBeforeFinalize,
        );
        phase5ActualSummary = `initiate + ${WITHDRAW_DELAY_SECS}s wait + finalize; payer +${fmtMUsd(mUsdAfterFinalize - mUsdBeforeFinalize)}`;
      } else {
        console.log(
