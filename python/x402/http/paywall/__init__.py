...
class EvmPaywallHandler:
    ...
    def generate_html(
        ...
        faucet_url = config.faucet_url if config else None
        ...
        if faucet_url:
            x402_config["faucetUrl"] = faucet_url
        if faucet_urls:
...
class PaywallBuilder:
    ...
    def with_config(
        ...
        Args:
            ...
            faucet_url: Global override for the testnet "Get some here" link.
...
class PaywallProvider:
    ...
    def generate_html(
        ...
        merged_config = PaywallConfigClass(
            ...
            faucet_url=(
                config.faucet_url if config and config.faucet_url is not None else self.faucet_url
            ),
            faucet_urls=(
                config.faucet_urls
